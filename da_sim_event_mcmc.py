...
if __name__ == '__main__':
    ...
    def get_sim(params):
        ...
        E, Ea_frac, xp, yp, zp, theta_p, phi_p, theta_a, phi_a, sigma_xy, sigma_z = params
        Ep = E*(1-Ea_frac)
        Ea = E*Ea_frac
        ...
        trace_sim.sims[1].initial_point = (xp,yp,zp)
    ...
    def get_init_walker_pos(direction):
        #initialize walkers with correct values in the sim to see if there are still convergence issues
        ...
        # Ep_guess = source_sim.sims[0].srim_table.get_energy_w_stopping_distance(track_length - sigma_guess) 
        # a_frac_guess = 1-Ep_guess/E_prior.mu
        Ea_frac_guess = 0.5
        assert Ea_frac_guess >0
        
        print('initial_guess:', (Ea, Ea_frac_guess, best_point, theta_1, phi_1, theta_2, phi_2, sigma_xy_guess))

        return [(Ea+ np.random.randn()*0.01, Ea_frac_guess+ np.random.randn()*0.01,
                            best_point[0]+ np.random.randn()*0.01,
                            best_point[1]+ np.random.randn()*0.01,
                            best_point[2]+ np.random.randn()*0.01,
                            theta_1+ np.random.randn()*0.01, phi_1+ np.random.randn()*0.01,
                            theta_2+ np.random.randn()*0.01, phi_2+ np.random.randn()*0.01,
                            sigma_xy_guess+ np.random.randn()*0.01, sigma_z_guess+ np.random.randn()*0.01,
                            ) for w in range(nwalkers)]

    # We'll track how the average autocorrelation time estimate changes
    directory = 'run%d_dalpha_sim_mcmc_init_walker_at_true_values/event%d'%(run_number, event_num)
