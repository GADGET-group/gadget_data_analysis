...
for run in production_runs:
    ...
    plt.hist2d(total_counts[mask], total_ranges[mask], 100, norm=mpl.colors.LogNorm(), range=[[0,1e6],[0,125]])
    plt.colorbar()
    plt.title('RvE Run_%04d'%run)
    plt.xlabel('Energy (ADC counts)')
    plt.ylabel('Range (mm)')
    plt.savefig('RvE_run%04d_veto_lt_150.png'%run)
    plt.close()
