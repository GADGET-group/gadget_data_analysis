...
class HistogramFitFrame(ttk.Frame):
    def __init__(self, parent, data):
        ...
        self.bins_entry.bind("<FocusOut>", lambda e: self.update_hist())
    ...
    def update_hist(self):
        ...
        self.hist_ax.clear()
        self.hist_ax.hist(self.data, bins=num_bins)
        if len(self.elements) > 0:
            ...
            guess *= bin_size
            fit *= bin_size
            self.hist_ax.plot(self.bin_centers, guess, label='guess')
            self.hist_ax.plot(self.bin_centers, fit, label='fit')
            self.hist_ax.legend()
            #draw residuals
