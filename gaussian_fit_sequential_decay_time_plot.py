...
def seq_decay_fit(x,a1,mu1,sigma1,a2,mu2,sigma2,m,b):
    gaus1 = a1*np.exp(-(x-mu1)**2/(2*sigma1**2))
    gaus2 = a2*np.exp(-(x-mu2)**2/(2*sigma2**2))
    line = m*x + b
...
for event_number in range(len(array_of_categorized_events_of_interest)):
    if array_of_categorized_events_of_interest[event_number] == event_type: # This can be be changed to 'Accidental Coin' to look at how the time difference of random events should looks
        ...
        if event_type == 'Accidental Coin':
            ...
            popt,pcov = scipy.optimize.curve_fit(seq_decay_fit,time_bins, summed_trace, p0=[10000, 160, 10, 10000, 350, 10, 0, 0], bounds=([7000,130,6,7000,130,6,-np.inf,-np.inf],[200000,200,20,200000,512,20,np.inf,np.inf]))
            y_fit = seq_decay_fit(time_bins, *popt)

            print(popt)
            plt.clf()
            plt.plot(time_bins, y_fit, color='red', label='Gaussian Fit on summed traces')
            plt.plot(time_bins, summed_trace, label='Summed traces', color='skyblue')
            plt.title('Gaussian Fit')
            plt.xlabel('Time bins (20 ns)')
            plt.ylabel('ADC Counts')
            plt.legend()
            plt.show()
            pad_time_diff_sum.append(np.abs(popt[1] - popt[4]))
        if event_type == 'RnPo Chain':
            ...
            # plt.ylabel("Counts")
            # plt.hist(test_sigmas,bins=256)
            # plt.show()
            
            plt.clf()
            plt.title('Event %d'%event_number)
            plt.xlabel("Time Difference (bins)")
            plt.ylabel("Counts")
            plt.hist(pad_time_diff,bins=256)
            plt.show()
