...
rscale, wscale, tscale, Dscale, vscale = 20, 3, 0.05, np.sqrt(0.09), 3000
zscale = 200
if exploit_symmetry:
    ijk_array = []
    ...
    def map_r(a_ij, r, t, w, cosphi=0):
        ...
        D, v, b = D*Dscale, v*vscale, b*wscale
    ...
    ijk_array = []
    for n in range(N+1):
        istart = 1 if force_0_to_0 else 0
...
if os.path.exists(fname):
    ...
    if exploit_symmetry:
        ...
        guess.append(2/wscale) #charge spreading, mm  
    if allow_beam_off_axis:
        guess.append(0)
        guess.append(0)
...
if exploit_symmetry:
    print('D, v, charge spreading width = ',a_ijk_best[-3:]*(Dscale, vscale, wscale))
