...
# this list is missing run 193, add it back in when you are done processing that run!
...
for run in production_runs:
    ...
    hist, bin_edges = np.histogram(counts, bins=bins, range=(energy_low, energy_high))
    ...
    total_ranges = np.concatenate([total_ranges,np.multiply(length,50/max_length_centroid)])
