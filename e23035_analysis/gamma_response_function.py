...
#bg_shift_func = lambda E: max(0, -1.777e-6 + 0.00483)
...
#f.param_bound_functions['bg_shift'] = lambda E: (bg_shift_func(E), bg_shift_func(E))
...
peaks = [#('60Zn', 670.3, 0.3, 665,677), #doesn't fit well, maybe another peak?
        ...
        ('60Ga', 2559.0, 0.8, 2551, 2564), ('60Ga', 3848.3, 0.7, 3840,3856), #from ENSDF
          ('60Ga', 2826, 0.2, 2820, 2834), ('60Ga', 2996.8, 0.2, 2991, 3005), #2021OR01
...
bg_shift_vs_mu = ROOT.TGraphErrors(
    len(mu), 
    np.array(mu, dtype=np.float64), 
    np.array(bg_shift, dtype=np.float64), 
    np.array(mu_err, dtype=np.float64), 
    np.array(bg_shift_err, dtype=np.float64)
)
fit_results.append(fitting_tools.fit_graph(bg_shift_vs_mu, "TMath::Max([0]*x + [1], 0.)", [0,0], [(-1, 1), (-1, 1)]))
