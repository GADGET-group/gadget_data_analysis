...
if __name__ == '__main__':
    ...
    source_sim = build_sim.create_multi_particle_event(experiment, run_number, event_num, ['4He'])
    ...
    source_sim.sims[0].phi = 0.785398
    # source_sim.sims[1].phi = 3.92699
    source_sim.sims[0].sigma_xy = 2.0
    source_sim.sims[0].sigma_z  = 3.0
    # source_sim.sims[0].sigma_xy = source_sim.sims[1].sigma_xy  = 2.0
    # source_sim.sims[0].sigma_z = source_sim.sims[1].sigma_z  = 3.0
    source_sim.num_trace_bins = 80
    source_sim.simulate_event()

    traces_to_fit = [source_sim.sim_traces[p] for p in source_sim.sim_traces]
    pads_to_fit = [p for p in source_sim.sim_traces]

    h5file = build_sim.get_rawh5_object(experiment, run_number)
    #set zmax to length of trimmed traces
    temp_sim = build_sim.create_multi_particle_event(experiment, run_number, event_num, ['4He'])
    zmax = temp_sim.num_trace_bins*temp_sim.zscale
    
    zmax = 340
    
    x_real, y_real, z_real, e_real = source_sim.get_xyze(threshold=h5file.length_counts_threshold, traces=source_sim.sim_traces)
    ...
    # for sim event only
    track_center = np.array([0,0,20])
    track_direction_vec = np.array([0.577,0.577,-0.577])
    print("track_center: ", track_center)
    ...
    def get_sim(params):
        ...
        E, xp, yp, zp, theta_p, phi_p, sigma_xy, sigma_z = params
        Ep = E# *(1-Ea_frac)
        # Ea = E*Ea_frac
        trace_sim = build_sim.create_multi_particle_event(experiment, run_number, event_num, ['4He'])
        trace_sim.sims[0].initial_energy = Ep
        # trace_sim.sims[1].initial_energy = Ea
        trace_sim.sims[0].initial_point = (xp,yp,zp)
        # trace_sim.sims[1].initial_point = (xp,yp,za)
        ...
        trace_sim.set_real_data(pads_to_fit, traces_to_fit, trim_threshold=100, trim_pad=10, pads_to_sim_select='observed')
    ...
    def log_priors(params, direction):
        ...
        return E_prior.log_likelihood(E) + np.log(np.abs(np.sin(theta_p))) #+ density_scale_prior.log_likelihood(rho_scale)
    ...
    def get_init_walker_pos(direction):
        ...
        sigma_guess = 2.5
        pos_ball_size = 1 # default value is 1
        ...
        E_prior.mu = np.random.randn()*E_prior.sigma + 6.
        
        print('initial_guess:', (E_prior.mu, Ea_frac_guess, best_point, theta, phi, sigma_guess))

        guess_sim = get_sim((E_prior.mu, best_point[0], best_point[1], best_point[2], theta, phi, sigma_guess, sigma_guess))
        #build_sim.open_gui(guess_sim)
                
        return [(max(E_prior.sigma*np.random.randn() + E_prior.mu,0), # Ea_frac_guess + np.random.randn()*0.01,
    ...
    directory = 'run%d_single_alpha_sim_mcmc/event%d'%(run_number, event_num)
