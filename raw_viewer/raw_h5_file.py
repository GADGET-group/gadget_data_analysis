...
def process_run(data_file_path, flat_lookup_path=None, settings_path=None, overwrite=False):
    '''
    Process a run, saving the processed data to a new file.
    If overwrite is False, ask if the user wants to overwrite a previously processed run.
    
    TODO: Figure out why the zscale is read in as a string and not a float.
    '''

    data = raw_h5_file(file_path=data_file_path, flat_lookup_csv=flat_lookup_path)
    ...
    directory_path, h5_fname = os.path.split(data_file_path)
    ...
    if  os.path.isdir(directory_path):
        if not overwrite:
            print('Export directory already exists: %s'%directory_path)
            return
        print('Export directory already exists, but will be overwritten: %s'%directory_path)
